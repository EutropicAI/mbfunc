...
def nnedi3_resample_kernel_vertical(
    ...
    last = nnedi3_rpow2_vertical(
        input, eTimes, 1, nsize, nns, qual, etype, pscrn, device, fast, flat_kernel, flat_a1, flat_a2, flat_taps
    )
...
def nnedi3_rpow2_vertical(
    ...
    if eTimes >= 1:
        last = nnedi3_dh(
            input, field, nsize, nns, qual, etype, pscrn, device, fast, flat_kernel, flat_a1, flat_a2, flat_taps
        )
        eTimes = eTimes - 1
        field = 0
    else:
    ...
    if eTimes >= 1:
        return nnedi3_rpow2_vertical(
            last, eTimes, field, nsize, nns, qual, etype, pscrn, device, fast, flat_kernel, flat_a1, flat_a2, flat_taps
        )
    else:
        return last
...
def nnedi3_dh(
    ...
    if (field == 0 or field == 1) and (fast and sbitPS > 8):
        ...
        nn = core.nnedi3cl.NNEDI3CL(
            input8, field=field, dh=True, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, device=device
        )
        lr = core.fmtc.resample(
            input,
        ...
        return mvf.LimitFilter(
            mvf.Depth(lr, depth=sbitPS, sample=sSType), mvf.Depth(nn, depth=sbitPS, sample=sSType), thr=1.0, elast=2.0
        )
    elif flat_kernel == "nnedi3":
        return core.nnedi3cl.NNEDI3CL(
            input, field=field, dh=True, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, device=device
        )
    else:
        nn = core.nnedi3cl.NNEDI3CL(
            input, field=field, dh=True, nsize=nsize, nns=nns, qual=qual, etype=etype, pscrn=pscrn, device=device
